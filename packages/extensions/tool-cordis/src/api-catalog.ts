  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Absolute extra `workspace-write` roots; empty grants nothing extra.',
        parameters: [],
      },
      {
    ...
    methods: [
      {
        ...
        throws: ['SelfDevelopmentRemoteError with `self-development/disabled` while the facade is disabled.', 'whatever the task-control service or a task journal rejects with, converted at the facade boundary into `self-development/core` (`details.code` keeps the original code).'],
      },
      {
      ...
      },
      {
        ...
        returns: 'the operation id the facade generated plus the core\'s result.',
        throws: ['SelfDevelopmentRemoteError with `self-development/disabled` or `self-development/config-invalid`.', 'whatever the task-control service rejects with, converted at the facade boundary into `self-development/core` (`details.code` keeps the original code).'],
      },
      {
      ...
      },
      {
        ...
        returns: 'the operation id the facade generated plus the core\'s result.',
        throws: ['SelfDevelopmentRemoteError with `self-development/disabled`, `self-development/config-invalid`, or `self-development/actor-forbidden`.', 'whatever the task-control service rejects with, converted at the facade boundary into `self-development/core` (`details.code` keeps the original code).'],
      },
      {
      ...
      },
      {
        ...
        returns: 'the operation id the facade generated plus the core\'s result.',
        throws: ['SelfDevelopmentRemoteError with `self-development/disabled` or `self-development/config-invalid`.', 'whatever the core or the runner rejects with, converted at the facade boundary into `self-development/core` (`details.code` keeps the original code).'],
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Integrate one allocated task\'s worktree into a project branch. Calls on one service instance serialize in memory; calls across processes serialize on the experiments root\'s integration lock. Git failures inside the integration are reported as a `failed` result, never thrown. Once the lock is held and before any rebase, a dirty worktree is snapshotted under `req.snapshot` when supplied — `git add -A` plus one unsigned, hook-free commit — or, absent one, fails the integration before touching anything; either way the snapshot commit id, once made, reaches every subsequent result as `snapshotCommit`. When `req.verify` is supplied, it runs once against the worktree after a rebase (when one was needed) and before the fast-forward, whether or not the baseline had moved; a rejection or a thrown error both report `verification-failed` and leave the target branch untouched.',
        parameters: [{ name: 'req', description: 'task id, target branch, actor, and the optional verification gate and snapshot identity.' }],
