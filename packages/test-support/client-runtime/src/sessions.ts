...
export class FixtureSession implements SessionFace {
  ...
  /**
   * Fail-loud stub; supply `stopAll` on the fixture's session face to exercise it.
   * @returns never — always throws.
   */
  stopAll(): never {
    throw new Error(`test session "${this.sessionId}": stopAll is not stubbed — supply it on the fixture's session face`)
  }
