...
`cancel` stops only the active turn and keeps the pending queue, which then resumes in FIFO order. `stopAll` is the full-task stop: it resolves or resumes the ordinary Session, cancels the active turn while discarding every pending inbox occurrence, and returns the discarded identities in inbox clear order (`SessionStopAllValue.discardedItemIds`, next-step items before next-turn items). Repeating `stopAll` is idempotent: an idle Session with nothing pending returns the same receipt shape with an empty discard list. The stop arms a full-stop flag carried by the `stopAll` Session projection (`{ stopped: boolean }`), visible in follow opening snapshots, control-stream projection frames, and the projection-cache read ladder. While the flag reads stopped, no automatic continuation — queued-consumption wakes, hook steering, and similar non-user deliveries — can enter a model step: the Agent loop ends such wakes as a blocked turn with no request, no consumed input, and no new user content, and the wake's claimed messages are discarded. The flag clears exactly when an explicit user message commits (`user/message` with a `user` source, from this prompt endpoint, the SDK, or headless delivery), after which ordinary delivery resumes. The enforcement is Host-live state that the projection mirrors and the projection cache carries across a restart: a Session whose flag survives a restart blocks continuations until the next explicit user message, and a Session whose cached flag is unavailable reads as not stopped.
