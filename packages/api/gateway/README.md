...
### Revocation disconnects accepted mux connections

`/api/remote.mux` upgrades pass Connection's authentication check, and the Gateway derives each connection's caller identity once from the upgrade request through `ctx.connection.callerOf` and stores it on the connection. Every Remote invocation on that connection — opening and consuming — runs inside `ctx.connection.caller.run(caller, ...)`, so a `@Remote` handler reads that connection's `sessionId`, `Host`-derived `loopback`, and bound `certificateSerial` with `ctx.connection.caller.current()`; without a mounted Connection service the mux keeps its previous behavior. The Gateway subscribes to `ctx.connection.onSessionsRevoked` and closes every connection whose session appears in a revocation (`sessions.revoke`, `certificates.revoke`, or `logout`) with WebSocket close code 4401 and reason `session revoked`, letting its logical streams settle first. Unsubscribing and closing all sockets happen when the upgrade effect is disposed.
...
- Closing a revoked session's mux connections aborts their logical streams like any socket loss; the browser reconnects automatically, and the new handshake is refused with 401 because the session no longer authenticates. Handlers must tolerate the resulting cancellation; the disconnect provides no replay or graceful drain.
