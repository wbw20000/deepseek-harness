...
**Budget preset.** `approveBudget`'s `preset: 'unlimited'` expands to `{ mode: 'time', durationMs: 86_400_000, phaseTimeoutMs: 600_000, maxStepsPerAttempt: 40, noProgressAttemptLimit: 5 }` before the core ever sees it; any field also set explicitly overrides that field's expanded default. `durationMs` — explicit or preset-expanded — is hard-capped at 24 hours; a larger value refuses with `self-development/config-invalid` regardless of `preset`.
