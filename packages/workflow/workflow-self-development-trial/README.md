...
3. **Workspace seed.** The data home's workspace registry (`storages/workspace.json`, the workspace domain's single-unit document as the default `storage-json` backend lays it out) gains the worktree as a workspace titled `<taskId> (trial)`, so the trial GUI opens on the worktree instead of asking for one. A missing registry is created with `initialized: false`, leaving the registry's own history bootstrap to attach the campaign's stored sessions; an existing registry gets the record prepended to its display order; a registry that already lists the worktree, or one carrying a pending mutation marker, is left alone. The outcome is one line in the trial log, and a registry the seed does not recognise is refused rather than overwritten — the line then says to add the workspace by hand, and the open continues.
4. **Port allocation.** The first free loopback port in `portRange`, skipping ports already handed to this process's own live instances. An exhausted range fails with `self-development/trial-port-exhausted`.
5. **Spawn.** `node apps/cli/lib/bin.js web --host 127.0.0.1 --port <port> --no-open` runs inside the worktree, `DSH_HOME` set to the resolved data home, detached into its own process group. The instance's URL is read off the first `dsh web: http://…` line in its output; a process that exits or never prints that line before `readyTimeoutMs` fails the open with `self-development/trial-start-failed`, and the group is torn down before the failure is raised.
...
When `autoOpen` is `true` (the default) and an events consumer is reachable — read structurally through `ctx.get('selfDevelopmentEvents')`, so this package never has to depend on the events package directly — the service subscribes to its notifications and calls `openTrial` on its own once a settled-campaign pass names a task — the events service publishes it as kind `awaiting-trial` under `origin: 'campaign'`; a single passed round (`origin: 'commit'`, same kind) and a completed merge (`origin: 'merge'`) never open a trial. An events consumer that mounts only after this service does is picked up too: the subscription is taken on the `internal/service` notification for `selfDevelopmentEvents` (and re-taken if the service is re-provided), because plugin activation order is not overlay row order — a field test never auto-opened a trial while the subscription was attempted only once, at construction.
...
- **Automatic open needs the events consumer to publish `campaign-passed`.** `@deepseek-ai/dsh-workflow-self-development-events` maps the facade's campaign result into that kind; a deployment without that consumer (or with a different event source) only ever opens trials through `openTrial` itself, or through a compatible source supplied via `internals.events`.
- **The workspace seed writes the `storage-json` single-unit layout only.** A deployment whose data home template selects another storage backend, or a per-record layout for the workspace domain, ignores the seeded file and its trial GUI still asks for a workspace; the record shape and version come from `@deepseek-ai/dsh-workspace`'s own spec, the document envelope from `storage-json`'s documented format.
