...
`verifyAttemptResult` 只接受针对当前任务、尝试、启动输入的源码与产物、计划摘要完全一致的完整运行：退出码为零、无信号、无超时、无取消，且每个必测用例的每条必测断言都执行并通过。零用例报告、跳过或缺失断言、超时、信号、取消和非零退出都不能通过。在时间截止时或之后完成的结果视为迟到，不能把任务移入 `awaiting-trial`；报告的阶段时长或步数超出批准的 `phaseTimeoutMs` 或 `maxStepsPerAttempt` 同样视为迟到。轮数与时间先到先停：预算先耗尽的限制触发停止，另一项的余量不构成继续许可。

通过的报告还必须携带实测内容摘要：受信执行器报告其实际测试过的开发后源码与产物的 `testedSourceDigest` 和 `testedArtifactDigest`，以及其实际执行的验收定义的 `acceptanceDefinitionDigest`。`task/passed` 提交、人工试用批准绑定的结果摘要覆盖这些摘要，因此批准指向的是被测试的内容，而不只是尝试的起点。每个摘要由受信执行器计算并对字节独立复核；本包从不读取 worktree，无法确认某个摘要确实描述被测字节，也不会仅凭摘要存在就批准验收定义。内容绑定不是测试质量的证明：摘要记录的是执行器的声明，并让事后的替换可以通过日志被检测出来。

每条能力证据都声明其来源种类：`machine` 表示证据由受信宿主自行取得，`human-presence` 表示由到场的用户在启动时确认。尝试在其持久的 `attempt/started` 日志记录中写入聚合来源——任一条为 `human-presence` 即记为 `human-presence`，否则记为 `machine`。`human-presence` 只证明有人在场，不构成无人值守运行所需的隔离。来源无效的证据项以 `SELF_DEV_CAPABILITY_MISSING` 拒绝启动；旧 schema 版本（1 或 2）写入的日志校验失败并进入 handoff。
...
- **日志 schema 版本 1 与 2 被拒** — 任务日志 schema 版本为 3。版本 3 要求每条已验证结果都携带实测内容摘要与验收定义摘要，因此版本 2 的日志——其通过结果只绑定启动输入摘要——在打开时校验失败并进入 handoff。对已提交的历史代不存在迁移或向下读取路径。
- **实测内容摘要只是记录的声明** — 实测内容摘要与验收定义摘要由受信执行器计算并对字节复核；本包从不读取 worktree。日志保存聚合后的结果摘要，而非完整报告。验证者需要另行保留的报告才能与该摘要比较；仅靠日志无法重建实测摘要，也不能阻止同用户改写。
