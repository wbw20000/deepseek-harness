...
import { ARTIFACT, BUDGET_ONE_ROUND, SOURCE, TASK_ID, fullCapabilitySource, header, makeTaskDir, openReadyTask, passingResult } from './helpers.ts'
...
it('reopens after a runner rejects with an empty error message', async () => {
  ...
  await expect(controller.startAttempt({
    ...header(revision, 'empty-error'), sourceDigest: SOURCE, artifactDigest: ARTIFACT,
...
it('replays the same operation revision before and after reopening', async () => {
  ...
  let launches = 0
  const request = {
    ...header(revision, 'success'), sourceDigest: SOURCE, artifactDigest: ARTIFACT,
...
it('records the original expected revision on every event of a budget update', async () => {
  ...
  const { controller, revision } = await openReadyTask(dir, clock, BUDGET_ONE_ROUND, fullCapabilitySource)
  await expect(controller.startAttempt({
    ...header(revision, 'failed'), sourceDigest: SOURCE, artifactDigest: ARTIFACT,
...
it('removes the external abort listener when an attempt settles', async () => {
  ...
  await controller.startAttempt({
    ...header(revision, 'listener'), sourceDigest: SOURCE, artifactDigest: ARTIFACT, signal: shutdown.signal,
...
it.each([
  ...
  const { controller, revision } = await openReadyTask(dir, clock, BUDGET_ONE_ROUND, fullCapabilitySource)
  await expect(controller.startAttempt({
    ...header(revision, 'incomplete-report'), sourceDigest: SOURCE, artifactDigest: ARTIFACT,
