...
| `acceptance` | The drafted acceptance definition: `{ "cases": [ { "caseId", "command": string[], "cwd"?, "timeoutMs", "assertions": [ { "assertionId", "kind": "exit-code" \| "stdout-includes" \| "file-exists" \| "file-includes", ... } ] } ] }`. Every `plan.requiredCases[].caseId` and `assertionIds` must be defined here; validated before the approval card is shown. |
