...
|---|---|---|
| `integrated` | Fast-forwarded; `commit`, `baseMoved`, and (when a snapshot was made) `snapshotCommit` reported. | Emits `merge-integrated`; runs the configured [upgrade](#upgrade) unless `upgrade.kind` is `none`. |
