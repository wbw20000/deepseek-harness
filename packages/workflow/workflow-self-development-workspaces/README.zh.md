...
| `setup` | 可选的 `{ command, timeoutMs }`，在每个新建 worktree 登记之前运行一次；缺省——默认值——完全跳过 setup。对本 harness 自身仓库的任务 worktree 来说（`git worktree add` 出来的 worktree 不带 `node_modules`），典型取值是 `{ command: ["pnpm", "install", "--offline", "--frozen-lockfile"], timeoutMs: 300000 }`。 |

相对路径、不是正有限整数的 `maxConcurrentTasks`、空的 `setup.command`，或不是正有限整数的 `setup.timeoutMs`，都会在构造时抛出带 `SELF_DEV_WORKSPACE_CONFIG_INVALID` 的 `SelfDevelopmentWorkspacesError`。taskId 必须是纯字母数字加点、连字符、下划线的单词，且不含 `..`，也不以 `.lock` 结尾；其余情况会在命名分支或目录之前抛出 `SELF_DEV_WORKSPACE_TASK_INVALID`。该配置检查不建立文件系统隔离，也不能保护目录免受同用户其他进程的影响。
...
| 错误码 | 抛出条件 |
|---|---|
| `SELF_DEV_WORKSPACE_CONFIG_INVALID` | 路径配置字段缺失、为空或为相对路径，`maxConcurrentTasks` 不是正有限整数，`setup.command` 为空，或 `setup.timeoutMs` 不是正有限整数。 |
...
| `SELF_DEV_WORKSPACE_SETUP_FAILED` | 配置的 `setup` 命令为空、无法启动、非零退出，或未在 `timeoutMs` 内结束；worktree 与数据目录会先被拆除。 |
