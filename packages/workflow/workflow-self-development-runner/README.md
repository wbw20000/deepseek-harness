...
[`verify.ts`](src/verify.ts) exports `verifyAcceptance(worktree, acceptancePath, config)`, a verify-only entry point over the same acceptor: it calls `loadAcceptance` and `runAcceptance` alone, with no headless executor, no evidence write, and no task-control core involved, for a caller that already has a worktree it wants judged by an acceptance definition — for example an integration's pre-fast-forward verification gate. `config` is the narrow slice of the deployment configuration this path needs — `experimentsRoot`, `killGraceMs`, and an optional overall `phaseTimeoutMs` — not the full runner `Config`; `acceptancePath` must still resolve outside `experimentsRoot`, exactly as `loadAcceptance` always requires, and a frozen test plan's coverage is never checked, since there is no plan in scope. It resolves `{ ok: true, report }` with the observed `AcceptanceRun` once the acceptor completed — a report whose cases failed their assertions is still a completed run, not a `verifyAcceptance` failure — or `{ ok: false, reason }` when the definition could not be loaded or a case could not be spawned or torn down; it never throws. Every case process receives the worktree itself as `DSH_HOME`, since this entry point runs no separately configured dsh agent. The mounted service exposes the same path as a method, `ctx.selfDevelopmentRunner.verifyAcceptance(worktree, acceptancePath, { phaseTimeoutMs? })`, filling in `experimentsRoot` and `killGraceMs` from its own validated config, for callers that reach the runner through `ctx.get` rather than importing the package — the chat package's merge gate is one.

The executor stops on stdout overflow rather than accepting a truncated success. Acceptance output overflow fails every assertion. Both helpers drain the direct child's pipes, kill remaining members of its process group, and wait for the group to disappear before returning. Final cleanup judges group ownership by the spawned leader's pid, the helper's own observation of that leader's exit, and the `ps -o lstart=` start time read at spawn: an `EPERM` group signal while the leader is no longer observable as ours is recorded as `pgidReused` in the run result instead of failing the run. An unconfirmed exit still rejects the run, including when an unreaped process remains visible or the fingerprinted leader is still ours. Acceptance paths and artifact ancestors are checked through the filesystem; artifact symlinks contribute their link text without reading their targets. These checks do not prevent a concurrent same-user writer from replacing files between observations.
...
- **`verifyAcceptance` skips frozen-plan coverage and evidence entirely** — it is a thinner path than a supervised attempt on purpose: no `checkAcceptanceCoversPlan` call, no launch record, no attempt evidence, no outcome file. A caller that needs those guarantees runs a real supervised attempt instead; this entry point only answers whether a definition's own cases pass against a worktree, for callers — such as an integration verification gate — with no supervised attempt in scope.
