...
One `SelfDevelopmentEvent` carries `taskId`, `kind`, `origin`, `sessionId`, `title`, `occurredAt`, and the projection `revision` after the commit. `origin` names the raw source the event was folded from — `commit` for a durable task-journal commit (the table below), `campaign` for the Remote facade settling a whole campaign, `merge` for the chat merge flow — because the `kind` vocabulary is shared across all three on purpose (a passed round and a passed campaign both leave the task `awaiting-trial`): a consumer that must tell a settled campaign apart from one round of it — the chat package's campaign-result notice, the trial service's automatic open — keys on `origin`, never on the fixed title text. `sessionId` is optional and always `undefined` here: a self-development task is not bound to a chat session, and the optional field keeps the shape consumable by session-bound notification consumers. Every title is a fixed English template: it carries only the round number and a reason from the closed vocabulary, and never the free-text failure reason or handoff detail.
