...
| `gcIntervalMs` | `0`（关闭） | 垃圾回收定时器间隔；设置后，每次回收都会咨询已注册的引用来源 |
| `gcGracePeriodMs` | `24 小时` | 垃圾回收定时器对未引用对象适用的宽限期 |
| `gcReferenceTimeoutMs` | `30 秒` | 一次异步引用来源读取的截止时限；仍挂起的来源使该轮跳过 |
...
`collectGarbage({ referenced, olderThanMs })` 删除附件 id 不在调用方 `referenced` 集合内、且最后修改时间早于 `olderThanMs` 的已存对象，返回回收的字节数与对象数。删除文件对象时也会一并移除其只读名称链接。回收绝不触碰被引用对象或宽限期内的任何对象，并且只在调用方触发时运行：`gcIntervalMs` 启用定时器，每一轮定时回收都会咨询用 `setGarbageReferenceSource()` 注册的引用来源，因为只有调用方知道哪些会话仍引用某个附件。随附的 `dsh` 组合默认关闭定时器；Session Controller 会注册引用来源，因此打开定时器即可让回收生效。

已注册的来源可以异步解析，例如读取持久化会话日志。以下情况会让该轮跳过、不删除任何对象：没有注册来源、来源返回或解析为 `undefined`、来源抛错，或来源在 `gcReferenceTimeoutMs` 之后仍未完成；跳过的一轮会记录告警。仍在读取中的一轮还会让下一个定时 tick 跳过，而不是与并发删除竞速。这种偏向跳过的设计是有意的：引用集不可读时，绝不能扩大一轮回收可删除的范围。
...
- **回收只覆盖会话引用**——harness 引用来源点名的是会话日志与待处理排队消息中记录的附件 id，因此被外部工具使用的附件对回收不可见；共享同一 home 的第二个 harness 进程对预算账本不可见，一个进程的启动清理也会把另一个活跃进程的预留记录当作孤儿。
...
`collectGarbage()` API 负责删除，Session Controller 已把会话引用接入定时器。恢复和 fork 后的会话可能共享不可变对象，因此引用集必须始终是整个会话语料的并集；会话日志之外的第三方工具引用不计入。服务于远程运行时或共享存储的后端还需要自己的持久性证明与预算账本。两个方向都尚未决定。
