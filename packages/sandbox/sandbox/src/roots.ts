 ...
 * deployment-configured extra writable roots (absolute execution-world paths;
 * absent when none are configured), the host `/tmp`, and the per-user
...
export function writableRoots(policy: SandboxExecutionPolicy): string[] {
  ...
  return [...new Set([policy.workspaceRoot, ...extra, '/tmp', tmpdir()].map(canonicalPath))]
}
