...
- **The Windows ACL rung refuses extra writable roots** — expressing a configured extra root needs the workspace's deterministic standing SID, and such an ACE stays on the directory after the deployment drops the root from its configuration, so no config change can revoke it. `confine()` therefore fails closed with an actionable error before any grant or child spawn when a `workspace-write` policy carries `extraWritableRoots`; the rung has no extra-root support until a revocable grant design exists.
