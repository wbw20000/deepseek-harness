...
- **Windows ACL 档拒绝额外可写根目录**——表达配置的额外根目录需要工作区的确定性常驻 SID，而部署从配置中移除该根目录后，此类 ACE 会一直留在目录上，任何配置变更都无法撤销。因此当 `workspace-write` 策略携带 `extraWritableRoots` 时，`confine()` 会在任何授权或子进程启动之前以可操作的错误 fail closed；在出现可撤销的授权设计之前，该档不支持额外根目录。
