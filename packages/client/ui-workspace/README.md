...
A deep link names one Session as the document path `/session/<id>`, where `<id>` is 1–128 characters from letters, digits, dot, underscore, and hyphen. When the page starts on such a path, the package waits for the Session list to become ready, selects that Session as the main view, and then rewrites the address back to `/` with `history.replaceState`, so the id never stays in the URL bar. This runs only inside an already-authenticated document: a `/session/<id>` request without a browser cookie receives the same authentication response as `/`, and the id is read only from the loaded document path. The startup route is consumed once; the ordinary startup selection (saved or most recent Workspace) fills in when the id cannot be adopted.

An id the ready Session list does not catalogue — unknown, renamed away, or not visible to this device — selects nothing: the shell overlay shows one transient localized banner, the refusal is logged to the console, and no exception escapes.

Native shells can skip URL navigation and call the bridge instead: `window.__DSH_BRIDGE__.selectSession(id)` returns a `Promise<boolean>` and is the only member the bridge object exposes. It accepts the same id form and answers `false`, with no side effects, before the Session list is ready and for any id outside the accepted form or absent from the ready list; it answers `true` only after the selection succeeded. The global appears when this plugin's client half initializes and is removed again with it; shells should wait for the global instead of probing before boot.
