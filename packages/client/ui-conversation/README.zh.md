...
“全部停止”不同于“停止生成”：停止生成只取消当前回合并保留排队，而全部停止（旁边的描边按钮，仅在回合运行或排队非空时可用）调用 Session 的 `stopAll` —— 取消当前回合、丢弃全部排队消息，并在用户再次发送前不自动开始新回合。全部停止会弹二次确认（说明将丢弃多少条排队消息），toast 显示丢弃数量；输入区上方可关闭的横幅镜像宿主的“已全停”状态，直到下一次发送将其清除。
