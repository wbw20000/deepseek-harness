...
The Client plugin provides `ctx.fileUpload`. Its `upload()` method receives the owning Session identity, assembles the raw route request, and invokes the generated Remote for exact bytes. A `Blob` declares its media type through the `x-dsh-file-type` request header so the Host can admit it against the deployment allowlist; the transport framing type stays `application/octet-stream`. The provider reads the optional pre-Cordis `__DSH_FILE_UPLOAD__` hook once. Without a hook, each Blob or stream request owns a short-lived Worker and releases it after completion, failure, or cancellation. With the hook, the service sends the body through the page-owned Fetch carrier; the Web Worker runtime transfers stream bodies through its request frame and exposes them to the Host HTTP bridge as backpressured chunks.

The Host plugin provides `ctx.fileUploads`. It owns the authenticated streaming route, encoded Remote fallback, command receipt resolver, and staged-receipt lifecycle; attachment storage, admission policy, and byte limits stay behind `ctx.attachments`. Before the route reads any request byte, it checks the declared `Content-Length` and declared file type against the mounted attachment store's policy and answers an over-limit declaration with HTTP 413 and an unaccepted declared file type with HTTP 415; the declared length and type then ride into storage, where the stream is counted so a lying or missing `Content-Length` cannot bypass the byte limit or the disk budget. Receipt tables use the receiving Agent's Session object as their key. The Session Controller registers the resolver that can resume a cold ordinary Agent and consumes receipts during prompt admission. Prompt delivery holds each receipt binding in a disposable transaction: disposal restores the previous binding until successful delivery commits it, and queue or history observation then retires the committed receipt.
...
- **Admission depends on the mounted attachment store** — the route maps only `FILE_TOO_LARGE` and `UNSUPPORTED_FILE_TYPE` to 413 and 415; byte counting and the disk budget apply during storage, where a mid-stream refusal returns the JSON failure envelope.
