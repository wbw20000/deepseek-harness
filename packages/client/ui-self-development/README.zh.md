...
| 停止 | 任何运行中的状态 | `stop` |
| 记录试用批准 | 待试用，绑定当前已验证结果 | `recordTrialApproval` |

**启动一轮**在勾选**我在电脑旁，本次为有人监督运行**之前保持禁用；勾选后请求携带 `presenceAcknowledged: true`，绝不默认填入。操作人自行输入 `confirmedBy`；worktree、产物、验收定义与数据目录路径来自稳定侧表单，并在对话框前回显。这里没有升级批准按钮。

### 未启用与手机白名单

组合未加载生成的 `selfDevelopmentRemote` 命名空间时，面板渲染"未启用"视图而不是报错；普通聊天组合不会产生任何授权。在手机上（非回环 Host 连接）白名单保留所有读操作与全部授权按钮，用一条提示隐藏试验版与证据路径，并禁用仅 Host 可见的 data-home 字段；手机上启动一轮同样要求在场勾选。
