...
describe('self-development wire builders', () => {
  ...
  it('builds the launch-profile input only when a required path is filled', () => {
    ...
    expect(buildLaunchProfileInput({ worktree: '/wt', acceptancePath: '', artifactPaths: '' })).toBeUndefined()
    expect(buildLaunchProfileInput({ worktree: '', acceptancePath: '/a.yml', artifactPaths: '' })).toBeUndefined()
  ...
  it('reads the launch profile and the trial approval off the card and projection views', () => {
    ...
    expect(trialApprovalOf(projection({ trialApproval: { approvedBy: 'mima', resultDigest: 'f'.repeat(64) } }))?.approvedBy).toBe('mima')
    expect(trialApprovalOf(projection())).toBeUndefined()
  })
