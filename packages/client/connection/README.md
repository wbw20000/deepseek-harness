...
- **The session registry is local to one process and one data directory** — registrations are not shared across processes or devices beyond the serial the trusted proxy reports; concurrent processes sharing one credentials file keep last-write-wins registry snapshots without merging. Revocation, including per-certificate revocation, is enforced only at DSH's own checks: a revoked session's already-accepted WebSocket mux connection keeps serving until it disconnects, because this package owns no live-connection table, and the checks do not constrain other same-user processes on the host, which the credentials file already trusts.
